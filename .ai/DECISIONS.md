...
## 2026-08-08 - 滯納金應繳總額為衍生顯示，定義待確認

### 決策

滯納金試算的「應繳總額」以 `申報稅額 + 滯納金` 即時計算並顯示，不儲存。

驗證訊息與欄位標籤統一為「法定期限」；`late_fee.date.required_pair` 與
`late_fee.date.range_invalid` 兩個字串同步更新。

### 理由

`services.late_fee` 沒有定義應繳總額，repository 也沒有對應欄位。它是為了讓使用者不必
自己加總而做的衍生顯示。

原本頁面標籤為「最後繳款日」而 i18n 訊息也是；stage 13 依需求草稿改為「法定期限」後，
驗證失敗會提到畫面上不存在的欄位名。修 i18n 兩個字串比讓頁面退回舊用詞更符合需求。

### 影響

- `[NEEDS CLARIFICATION]` 應繳總額的定義是否僅為「申報稅額 + 滯納金」？逾期超過 30 日
  後是否應納入滯納利息或其他款項？若定義不同，此列應改算或移除，而不是留著一個意義
  未定的數字。
- 在確認前，該列僅為便利性加總，不得作為對外請款或帳務依據。
...
暫不改變 `DateField.set_date_range` 的語意。範圍目前只套用到日曆彈窗的可選日期，
`validated_value()` 不因超出範圍而拒絕輸入。

### 理由

實測（stage 5 重做時）：`set_date_range` 在 `src` 全域沒有任何呼叫者，儲存的範圍也沒有
任何程式碼讀取。它既不是導覽輔助，也不是驗證規則——兩者都不是。

在無呼叫者的情況下選定其中一種語意，等於替六個未來的呼叫端決定驗證行為，而
`validated_value()` 的回傳契約會因此改變。這屬於資料語意，不由實作端補齊。

### 影響

- `[NEEDS CLARIFICATION]` 當畫面設定日期範圍後，`validated_value()` 應否對「格式正確
  但超出範圍」的日期拋出 `InvalidInput`？或者範圍僅供日曆導覽，驗證仍只看格式？
- 在答案出現前，任何新畫面不應依賴 `set_date_range` 做驗證。需要範圍驗證的畫面
  自行在送出前檢查，並在該處顯示錯誤。
